...
import { regionsGet } from "latitudesh-typescript-sdk/funcs/regionsGet.js";
...
async function run() {
  const res = await regionsGet(latitudesh, {});
  if (res.ok) {
    ...
    console.log("regionsGet failed:", res.error);
  }
}
...
```typescript
import { LatitudeshCore } from "latitudesh-typescript-sdk/core.js";
import { regionsFetch } from "latitudesh-typescript-sdk/funcs/regionsFetch.js";
...
async function run() {
  const res = await regionsFetch(latitudesh, {
    regionId: "reg_GMy1DbYLqN50m",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("regionsFetch failed:", res.error);
  }
}
...
**Promise\<[models.Region](../../models/region.md)\>**
