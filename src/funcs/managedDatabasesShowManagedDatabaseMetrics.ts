...
export function managedDatabasesShowManagedDatabaseMetrics(
  ...
  Result<
    operations.ShowManagedDatabaseMetricsResponse,
    | LatitudeshError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ShowManagedDatabaseMetricsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ShowManagedDatabaseMetricsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
