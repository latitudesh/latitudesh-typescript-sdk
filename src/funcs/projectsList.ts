...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetProjectsRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    "extra_fields[projects]": payload?.["extra_fields[projects]"],
    "filter[billing_type]": payload?.["filter[billing_type]"],
    "filter[description]": payload?.["filter[description]"],
    "filter[environment]": payload?.["filter[environment]"],
    "filter[name]": payload?.["filter[name]"],
    "filter[slug]": payload?.["filter[slug]"],
