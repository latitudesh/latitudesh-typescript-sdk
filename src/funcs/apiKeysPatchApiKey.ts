 ...
 */
export function apiKeysPatchApiKey(
  client: LatitudeshCore,
  request: operations.PatchApiKeyRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchApiKeyResponse,
    | LatitudeshError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchApiKeyRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/auth/api_keys/{api_key_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchApiKeyResponse$inboundSchema, {
