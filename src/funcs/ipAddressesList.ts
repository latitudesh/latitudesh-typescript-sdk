...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetIpsRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "filter[available]": payload?.["filter[available]"],
    "filter[family]": payload?.["filter[family]"],
    "filter[location]": payload?.["filter[location]"],
    "filter[management]": payload?.["filter[management]"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetIpsResponse$inboundSchema, {
