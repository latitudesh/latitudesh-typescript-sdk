...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTrafficConsumptionRequest$outboundSchema.parse(value),
  ...
  const query = encodeFormQuery({
    "filter[date][gte]": payload["filter[date][gte]"],
    "filter[date][lte]": payload["filter[date][lte]"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.Traffic$inboundSchema, {
