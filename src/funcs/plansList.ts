...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetPlansRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "filter[in_stock]": payload?.["filter[in_stock]"],
    "filter[location]": payload?.["filter[location]"],
    "filter[name]": payload?.["filter[name]"],
    "filter[ram]": payload?.["filter[ram]"],
    "filter[slug]": payload?.["filter[slug]"],
    "filter[stock_level]": payload?.["filter[stock_level]"],
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPlansResponse$inboundSchema, {
