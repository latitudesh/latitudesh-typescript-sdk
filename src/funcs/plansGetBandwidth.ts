...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetBandwidthPlansRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const headers = new Headers(compactMap({
    ...
    "API-Version": encodeSimple("API-Version", payload?.["API-Version"], {
      explode: false,
      charEncoding: "none",
    }),
  }));
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetBandwidthPlansResponse$inboundSchema, {
