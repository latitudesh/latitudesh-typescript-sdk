...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetServersRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "filter[ram][lte]": payload?.["filter[ram][lte]"],
    "filter[region]": payload?.["filter[region]"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetServersResponse$inboundSchema, {
