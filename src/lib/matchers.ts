...
import { LatitudeshDefaultError } from "../models/errors/latitudeshdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LatitudeshDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LatitudeshDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LatitudeshDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LatitudeshDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
